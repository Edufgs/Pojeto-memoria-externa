...
boolean mockupSerial = false; //false para pegar dados da serial
...
// plots
Graph BarChart = new Graph(225, 70, 800, 150, color(20, 20, 200));  //Configuração do grafico de barra Graph(y,?,x,?)
Graph LineGraph = new Graph(225, 350, 800, 200, color (20, 20, 200)); //Configuração do grafico de linha Graph(y,?,x,?)
float[] barChartValues = new float[6];
float[][] lineGraphValues = new float[6][253]; //Numeros de pontos
float[] lineGraphSampleNumbers = new float[253]; //Numeros de pontos na tela
...
void setup() {
  surface.setTitle("Grafico de Valores");
  size(1090, 820); //Padrão 890px por 620px
  ...
  //coloca o botão "Receber"
  //Primeira imagem é a cor, segunda imagem é quando passa o mouse por cima e a terceira é quando clica no botão
  PImage[] imgs = {loadImage("Imagens\\button_a.png"),loadImage("Imagens\\button_b.png"),loadImage("Imagens\\button_c.png")};
  cp5.addButton("receber")
     .setValue(128)
     .setPosition(447.5,740)
     .setImages(imgs)
     .updateSize()
     ;
     
  //Texto com a data e hora
  data = cp5.addTextlabel("data")
                    .setText("Data:")
                    .setPosition(10,630)
  ...
  hora = cp5.addTextlabel("hora")
                    .setText("Hora:")
                    .setPosition(10,690)
                    .setColorValue(#000000)
                    .setFont(createFont("Arial",20))
                    ;
  //texto("Dia/mes/ano, diaSemana", "00:00:00");
...
void draw() {
  byte[] inBuffer = new byte[150]; // holds serial message
  ...
  if (mockupSerial || serialPort.available() > 0) {
    ...
    if(nums.length == 4){
        data.setText("Data: "+semana(nums[1])+", dia "+nums[0]);
        hora.setText("Hora: "+nums[2]);
    }else{
...
//Transforma um numero em dia da semana
String semana(String s){
  switch (s) {
        case "0":
            return "Sábado";
        case "1":
...
//Função responsavel pelo botão "Receber"
public void receber() {
  if(botao){
    cont = 0;
    println("Botão Precionado");
    serialPort.write('A');
  }  
}
